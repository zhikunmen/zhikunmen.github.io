class yefan {
	...
	public static init() {
		...
		let timeKeys = [];
		let childrenItems = [];
		let arrange = timeArr.data.arrange
		for (var timeKey in arrange) {//拿到周几日期 key值
			timeKeys.push(timeKey);
			var items = [];
			for (let i = 0; i < arrange[timeKey].length; i++) {
				let objItem = { label: arrange[timeKey][i].start_time, value: arrange[timeKey][i].id };
				items.push(objItem);
			}
			childrenItems.push(items);
		}
		for (let i = 0; i < timeKeys.length; i++) {
			let objItem = { label: timeKeys[i], children: childrenItems[i] };
			obj.push(objItem);
		}
		console.log(obj);
	}
